...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_LED_LIGHT, DP_TYPE_BOOL},
  {DPID_WATER_TEMP, DP_TYPE_VALUE},
  {DPID_WATER_CIRCULATING, DP_TYPE_BOOL},
  {DPID_AIRTEMP, DP_TYPE_VALUE},
  {DPID_MONDAY, DP_TYPE_BOOL},
  {DPID_TUESDAY, DP_TYPE_BOOL},
  {DPID_WEDNESDAY, DP_TYPE_BOOL},
  {DPID_THURSDAY, DP_TYPE_BOOL},
  {DPID_FRIDAY, DP_TYPE_BOOL},
  {DPID_SATURDAY, DP_TYPE_BOOL},
  {DPID_SUNDAY, DP_TYPE_BOOL},
  {DPID_BAT_LOW_SET, DP_TYPE_VALUE},
  {DPID_MANUAL_FEED_ONCE, DP_TYPE_BOOL},
  {DPID_TIME_H, DP_TYPE_VALUE},
  {DPID_TIME_MIN, DP_TYPE_VALUE},
};
...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_FEED_REPORT,当前当天已喂食); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_LED_LIGHT,当前补光灯); //BOOL型数据上报;
    mcu_dp_value_update(DPID_WATER_TEMP,当前水温度); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_WATER_CIRCULATING,当前水循环); //BOOL型数据上报;
    mcu_dp_value_update(DPID_AIRTEMP,当前环境温度); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_MONDAY,当前周一); //BOOL型数据上报;//补光灯
    mcu_dp_bool_update(DPID_TUESDAY,当前周二); //BOOL型数据上报;//水泵
    mcu_dp_bool_update(DPID_WEDNESDAY,当前周三); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_THURSDAY,当前周四); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_FRIDAY,当前周五); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_SATURDAY,当前周六); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_SUNDAY,当前周日); //BOOL型数据上报;
    mcu_dp_value_update(DPID_BAT_LOW_SET,当前电池电量低); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_MANUAL_FEED_ONCE,当前手动喂食1次); //BOOL型数据上报;
    mcu_dp_value_update(DPID_TIME_H,当前喂食时间—时); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TIME_MIN,当前喂食时间-分); //VALUE型数据上报;
    */
...
/*****************************************************************************
函数名称 : dp_download_led_light_handle
功能描述 : 针对DPID_LED_LIGHT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_led_light_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    led_light = mcu_get_dp_download_bool(value,length);
    if(led_light == 0) 
    {
        //开关关
        //使用周一代替
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_LED_LIGHT,led_light);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_water_circulating_handle
功能描述 : 针对DPID_WATER_CIRCULATING的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_water_circulating_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char water_circulating;
    
    water_circulating = mcu_get_dp_download_bool(value,length);
    if(water_circulating == 0) 
    {
        //开关关
        //使用周二代替
        //JDQ_Water = Off;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_WATER_CIRCULATING,water_circulating);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_monday_handle
功能描述 : 针对DPID_MONDAY的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_monday_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    monday = mcu_get_dp_download_bool(value,length);
    if(monday == 0) 
    {
        //开关关
        LED1 = Off;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_MONDAY,monday);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_tuesday_handle
功能描述 : 针对DPID_TUESDAY的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_tuesday_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    tuesday = mcu_get_dp_download_bool(value,length);
    if(tuesday == 0) 
    {
        //开关关
        JDQ_Water = Off;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_TUESDAY,tuesday);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_wednesday_handle
功能描述 : 针对DPID_WEDNESDAY的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_wednesday_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    wednesday = mcu_get_dp_download_bool(value,length);
    if(wednesday == 0) 
    {
        //开关关
    }else 
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_WEDNESDAY,wednesday);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_thursday_handle
功能描述 : 针对DPID_THURSDAY的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_thursday_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    thursday = mcu_get_dp_download_bool(value,length);
    if(thursday == 0) 
    {
        //开关关
    }else 
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_THURSDAY,thursday);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_friday_handle
功能描述 : 针对DPID_FRIDAY的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_friday_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    friday = mcu_get_dp_download_bool(value,length);
    if(friday == 0) 
    {
        //开关关
    }else 
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_FRIDAY,friday);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_saturday_handle
功能描述 : 针对DPID_SATURDAY的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_saturday_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    saturday = mcu_get_dp_download_bool(value,length);
    if(saturday == 0) 
    {
        //开关关
    }else 
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SATURDAY,saturday);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_sunday_handle
功能描述 : 针对DPID_SUNDAY的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_sunday_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    sunday = mcu_get_dp_download_bool(value,length);
    if(sunday == 0) 
    {
        //开关关
    }else 
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SUNDAY,sunday);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_bat_low_set_handle
功能描述 : 针对DPID_BAT_LOW_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_bat_low_set_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long bat_low_set;
    
    bat_low_set = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_BAT_LOW_SET,bat_low_set);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_manual_feed_once_handle
功能描述 : 针对DPID_MANUAL_FEED_ONCE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_manual_feed_once_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char manual_feed_once;
    
    manual_feed_once = mcu_get_dp_download_bool(value,length);
    if(manual_feed_once == 0) 
    {
        //开关关
        JDQ_FishFood = ON;
    }else 
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_MANUAL_FEED_ONCE,manual_feed_once);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_time_h_handle
功能描述 : 针对DPID_TIME_H的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_time_h_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long time_h;
    
    time_h = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_TIME_H,time_h);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_time_min_handle
功能描述 : 针对DPID_TIME_MIN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_time_min_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long time_min;
    
    time_min = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_TIME_MIN,time_min);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_LED_LIGHT:
            //补光灯处理函数
            ret = dp_download_led_light_handle(value,length);
        break;
        case DPID_WATER_CIRCULATING:
            //水循环处理函数
            ret = dp_download_water_circulating_handle(value,length);
        break;
        case DPID_MONDAY:
            //周一处理函数
            ret = dp_download_monday_handle(value,length);
        break;
        case DPID_TUESDAY:
            //周二处理函数
            ret = dp_download_tuesday_handle(value,length);
        break;
        case DPID_WEDNESDAY:
            //周三处理函数
            ret = dp_download_wednesday_handle(value,length);
        break;
        case DPID_THURSDAY:
            //周四处理函数
            ret = dp_download_thursday_handle(value,length);
        break;
        case DPID_FRIDAY:
            //周五处理函数
            ret = dp_download_friday_handle(value,length);
        break;
        case DPID_SATURDAY:
            //周六处理函数
            ret = dp_download_saturday_handle(value,length);
        break;
        case DPID_SUNDAY:
            //周日处理函数
            ret = dp_download_sunday_handle(value,length);
        break;
        case DPID_BAT_LOW_SET:
            //电池电量低处理函数
            ret = dp_download_bat_low_set_handle(value,length);
        break;
        case DPID_MANUAL_FEED_ONCE:
            //手动喂食1次处理函数
            ret = dp_download_manual_feed_once_handle(value,length);
        break;
        case DPID_TIME_H:
            //喂食时间—时处理函数
            ret = dp_download_time_h_handle(value,length);
        break;
        case DPID_TIME_MIN:
            //喂食时间-分处理函数
            ret = dp_download_time_min_handle(value,length);
        break;
